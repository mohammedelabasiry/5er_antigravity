export const translations = {
  en: {
    // Navbar
    navDashboard: "Dashboard",
    navMapDiscovery: "Map Discovery",
    navBeneficiarySearch: "Beneficiary Search",
    navMessages: "Messages",
    navHistory: "History",
    navResources: "Resources",
    navAuditReports: "Audit & Reports",
    navLogout: "Log out",
    navLogin: "Log in",
    navSignup: "Sign up",
    navGovernance: "Governance",

    // Common
    welcome: "Assalamu Alaikum",
    save: "Save",
    cancel: "Cancel",
    confirm: "Confirm",
    pending: "Pending",
    delivered: "Delivered",
    failed: "Failed",
    egp: "EGP",
    anonymous: "Anonymous",
    loading: "Loading...",
    status: "Status",
    date: "Date",
    details: "Details",
    confirmed: "Confirmed",
    cancelled: "Cancelled",
    rejected: "Rejected",
    en_route: "En Route",
    transactions: "Transactions",
    
    // Landing Page
    heroTitle: "Transparent Sadaqah & Aid Governance",
    heroSubtitle: "KhairLink connects donors, beneficiaries, and charities in a secure, privacy-first, and auditable network with monthly support limits.",
    getStarted: "Get Started",
    learnMore: "Learn More",
    impactTitle: "Transparent Impact Realtime Registry",
    beneficiariesRegistered: "Beneficiaries Registered",
    totalDonations: "Total Donations Routed",
    deliveryRate: "Logistics Delivery Rate",
    
    // Chat
    secureShield: "Secure Shield Enabled",
    typeMessage: "Type your message securely...",
    supportChannels: "Support Channels",
    noChats: "No chats active",
    safetyNotice: "Privacy Governance: Direct contact details (phones, emails, IDs) are programmatically redacted/blocked.",
    
    // Map
    mapPortal: "Map Discovery Portal",
    searchRadius: "Search Radius",
    category: "Category",
    familyDetails: "Family Details",
    remainingNeed: "Remaining Need",
    donateDirect: "Donate Direct",
    anonymousDetails: "Anonymous Details",
    privacyShielded: "Privacy shielded: All beneficiary locations have a random 100-300m offset.",
    matches: "Matches",
    resetFilters: "Reset Filters",
    onlyShowNeeding: "Only show cases needing support",
    
    // Transactions
    ledgerTitle: "Financial & Resource Audit Ledger",
    ledgerSubtitle: "Transparent transaction registry matching aid distributions with active support caps.",
    totalRegistry: "Total Registry Count",
    totalTransactedValue: "Total Transacted Value",
    pendingGovernance: "Pending Governance Check",

    // Dashboards Common & Donor
    welcomeBack: "Alhamdulillah, welcome back",
    welcomeName: "Welcome",
    donorPrivacyNotice: "Your donations are tracked transparently in coordinate boundaries for donor privacy.",
    discoverCases: "Discover Cases Near You",
    totalContributed: "Total Contributed",
    directSupportFunds: "Direct support funds",
    confirmedPending: "Confirmed & pending donations",
    directImpact: "Direct Impact",
    familiesSupported: "Families Supported",
    activeUrgentCases: "Active Urgent Support Cases",
    browseMap: "Browse Map",
    allTargetsCovered: "All monthly support targets are fully covered!",
    viewDetails: "View Details",
    support: "Support",
    contributionsHistory: "Your Contributions History",
    noDonationsYet: "You haven't made any donations yet.",
    startExploring: "Start by exploring verified local cases nearby.",
    uniqueCasesSupported: "Unique cases supported",

    // Beneficiary Dashboard
    caseId: "Case ID",
    chatRoom: "Chat Room",
    pendingAuditTitle: "Account Pending Governance Audit",
    pendingAuditDesc: "Your submitted documents are currently being checked by platform administrators. Once verification completes and your profile status changes to Approved, you will become searchable by local donors and organizations.",
    rejectedTitle: "Application Rejected",
    rejectedDesc: "Unfortunately, your need evaluation registration was rejected as it did not satisfy platform need criteria or contains incorrect information. Please contact admin support for details.",
    monthlySupportTracker: "Monthly Support Tracker",
    currentMonth: "Current Month",
    progressToTarget: "Progress to target",
    percentMet: "Met",
    monthlyCapTarget: "Monthly Cap Target",
    receivedAid: "Received Aid",
    remainingNeeded: "Remaining Needed",
    socioEconomicScore: "Socio-Economic Score",
    assignedBracket: "Assigned Bracket",
    extremelyVulnerable: "Extremely vulnerable",
    veryNeedy: "Very needy",
    needy: "Needy",
    limitedSupport: "Limited support",
    familyMembers: "Family Members",
    childrenUnder18: "Children under 18",
    employmentStatus: "Employment Status",
    rentBurden: "Rent Burden",
    contributionsLedger: "Contributions Ledger",
    noContributionsYet: "No contributions recorded yet for your case.",
    source: "Source",
    amount: "Amount",
    resourceDistributions: "Resource Distributions",
    noResourcesYet: "No physical packages dispatched to your address.",
    itemType: "Item Type",
    qty: "Qty",
    notifications: "Notifications",
    noNotifications: "No alerts in your inbox.",
    locationUpdatedSuccess: "Location and map visibility updated successfully.",
    mapVisibilityTitle: "Map Visibility & Discovery Settings",
    showOnMapLabel: "Show my case on the public discovery map",
    showOnMapDesc: "If enabled, donors can find your case approximately on the map. Your exact location is always hidden.",
    updateSettingsBtn: "Update Settings",
    currentLocation: "Current Location Area",
    executeCampaign: "Launch Distribution Campaign",
    campaignSettings: "Campaign Target Filters",
    inventorySettings: "Inventory & Warehouse Stock",
    qtyPerFamily: "Qty per family",
    forceCapOverride: "Force override cap limit (Distribute even if monthly limit exceeded)",
    deductStock: "Deduct from inventory stock",
    runCampaign: "Run Campaign",
    campaignSuccess: "Campaign executed successfully!",
    stockAdjustment: "Quick Stock Intake / Adjustment",
    addStockItem: "Add New Stock Category",

    // Reports Dashboard
    governanceEngine: "Platform Governance Engine",
    auditLogsAnalytics: "Audit Logs & Analytics",
    charityLogsDesc: "Reviewing activity logs and support analytics for your organization.",
    adminLogsDesc: "Supervising platform-wide donation routing, support caps, and system audit logs.",
    beneficiaryReach: "Beneficiary Reach",
    approvedActiveCases: "Approved active support cases",
    cashSupportLedger: "Cash Support Ledger",
    directDonationsProcessed: "direct donations processed",
    physicalAidValue: "Physical Aid Value",
    distributionsCompleted: "supply distributions completed",
    totalAidValueRouted: "Total Aid Value Routed",
    directSupportLogistics: "Direct support and logistics value",
    vulnerabilityRatio: "Vulnerability Category Ratio",
    scoringDefinition: "Scoring Definition",
    scoringDefinitionDesc: "Category scores are calculated dynamically from family status, income level, debts, health issues, and housing conditions.",
    auditTrail: "Governance Audit Trail (Latest 50 actions)",
    timestamp: "Timestamp",
    actionEvent: "Action Event",
    operator: "Operator",
    operatorDetails: "Details",
    noAuditLogs: "No audit logs found matching criteria.",

    // Resource Logistics
    resourceLogisticsPortal: "Resource Logistics Portal",
    resourceLogisticsDesc: "Track and verify the distribution of physical resources (food crates, medicine, supplies) to registered beneficiary families.",
    noResourceDistributions: "No resource distributions recorded",
    noResourceDistributionsDesc: "There are no physical goods deliveries pending or completed in the system.",
    resourceItem: "Resource Item",
    beneficiaryFamily: "Beneficiary Family",
    logisticsValue: "Logistics Value",
    estimatedMarketValue: "Estimated Market Value",
    assignedDate: "Assigned",
    distributingEntity: "Distributing Entity",
    logisticsNotes: "Logistics Notes",
    updateLogisticsStatus: "Update Logistics Status",
    statusNotes: "Status Notes",
    addTrackingNotes: "Add tracking notes...",
    deliveryStatus: "Delivery Status",
    enRoute: "En Route",
    deliveryFailed: "Delivery Failed",
    refReferenceId: "Reference / ID",
    refBeneficiary: "Beneficiary",
    refSourceContributor: "Source Contributor",
    refTypeParticulars: "Type / Particulars",
    refValueEgp: "Value (EGP)",

    // Admin Dashboard
    systemAdmin: "System Administration",
    adminWelcome: "Welcome Back",
    adminTagline: "Governance Layer: Sadaqah target checks, case audit approvals, and fraud protection.",
    adminCore: "Admin Core",
    cases: "Cases",
    pendingVerification: "pending verification",
    totalRaised: "Total Raised",
    acrossCashResources: "Across cash & resources",
    charities: "Charities",
    pendingLicensing: "pending licensing review",
    donors: "Donors",
    activeContributors: "Active contributors",
    approvalQueue: "Approval Queue",
    auditCase: "Audit Case",
    verificationQueueClear: "Verification Queue Clear",
    verificationQueueClearDesc: "All registered profiles have been audited.",
    recentAuditLog: "Recent Audit Log trail",
    beneficiariesDirectory: "Beneficiaries Directory",
    code: "Code",
    displayName: "Display Name",
    legalName: "Legal Name",
    targetCap: "Target Cap",
    view: "View",
    navCharities: "Charities",
    navDonors: "Donors",

    // Admin Charities Directory
    charitiesDirectory: "Charity Organizations Directory",
    charitiesDirectoryDesc: "Review charity licenses, details, and manage platform permission states.",
    backToAdminDashboard: "Back to Admin Dashboard",
    searchCharityPlaceholder: "Search by charity name or license ID...",
    noCharitiesMatch: "No charity organizations match your query.",
    licenseId: "License",
    accountEmail: "Account email",
    registered: "Registered",
    suspendAccess: "Suspend Access",
    approveOrganization: "Approve Organization",
    pendingVerificationBadge: "Pending Verification",
    approvedBadge: "Approved",

    // Admin Donors Directory
    donorsDirectory: "Donors Directory",
    donorsDirectoryDesc: "Manage platform donors, review contribution tallies, and toggle access states.",
    searchDonorPlaceholder: "Search donors by name or email address...",
    noDonorsMatch: "No donors match your search.",
    totalDonated: "Total Donated",
    contributions: "Contributions",
    joined: "Joined",
    suspendedBadge: "Suspended",
    activeBadge: "Active",
    reactivateAccount: "Reactivate Account",
    suspendDonor: "Suspend Donor",

    // Admin Beneficiary Detail Page
    profileNotFound: "Profile not found",
    caseSummaryPublic: "Case Summary & Public Profile",
    displayNameLabel: "Display Name",
    generalNeighborhood: "General Neighborhood",
    caseStory: "Case Story",
    confidentialProfile: "Confidential Legal Profile",
    nationalId: "National ID",
    phone: "Phone",
    email: "Email",
    exactAddress: "Exact address",
    contributionsLedgerAdmin: "Contributions Ledger",
    contributor: "Contributor",
    type: "Type",
    noContributionsCase: "No contributions registered for this case yet.",
    physicalResourceDists: "Physical Resource Distributions",
    quantity: "Quantity",
    estimatedValue: "Estimated Value",
    deliveryStatusLabel: "Delivery Status",
    noDistributions: "No physical distributions registered.",
    monthlySupportCycle: "Monthly support cycle",
    supportProgress: "Support Progress:",
    auditLogsComments: "Audit Logs & Comments",
    noNotesLogged: "No notes have been logged for this case.",
    modifyCaseParameters: "Modify Case Parameters",
    caseCode: "Case Code",
    overrideActive: "OVERRIDE ACTIVE",

    // Admin Review Page
    auditPending: "Audit Pending",
    reviewCase: "Review",
    algorithmicAssessment: "Algorithmic Assessment",
    recommendedCap: "Recommended",
    caseSummarySection: "Case Summary & Public Profile",
    caseStoryLabel: "Case Story / Summary",
    confidentialLegalProfile: "Confidential Legal Profile (Admin Only)",
    contactPhone: "Contact Phone",
    homeAddress: "Home Address",
    socioEconomicMetrics: "Socio-Economic Evaluation Metrics",
    monthlyIncome: "Monthly Income",
    dependents: "Dependents",
    childrenUnder18Short: "Children under 18",
    employment: "Employment",
    housingTenure: "Housing Tenure",
    monthlySupportReceived: "Monthly Support Received",
    outstandingDebts: "Outstanding Debts",
    chronicIllnesses: "Chronic Illnesses / Medical State:",
    submittedDocuments: "Submitted Verification Documents",
    downloadScan: "Download Scan",
    governanceDecision: "Governance Decision Board",
    decisionAction: "Decision Action",
    approveCase: "Approve Case",
    rejectApplication: "Reject Application",
    needCategoryBracket: "Need Category Bracket",
    categoryA: "Category A - Extremely Vulnerable",
    categoryB: "Category B - Very Needy",
    categoryC: "Category C - Needy",
    categoryD: "Category D - Limited Support",
    monthlyCapSupport: "Monthly Cap support (EGP)",
    adminCapOverride: "Admin Cap Override",
    adminCapOverrideDesc: "If checked, this case is permitted to receive donations exceeding the monthly cap limit without being hidden.",
    auditorComments: "Auditor Comments / Review Notes",
    documentReasonPlaceholder: "Document the reasons for this decision or guidelines for social workers...",
    commitAuditVerdict: "Commit Audit Verdict",
    decisionAuditTrail: "Decision Audit Trail",

    // Charity Dashboard
    charityPortal: "Charity Organization Portal",
    licensePrefix: "License",
    approved: "✓ Approved",
    pendingApproval: "⏳ Pending Approval",
    searchBeneficiaries: "Search Beneficiaries",
    totalContributedCharity: "Total Contributed",
    transactionsLabel: "Transactions",
    familiesReached: "Families Reached",
    activeCasesNeedingSupport: "Active Cases Needing Support",
    allCasesFullySupported: "All cases are fully supported!",
    contributionHistory: "Contribution History",
    noContributions: "No contributions yet.",

    // Donate Page
    backToCaseProfile: "Back to Case Profile",
    contributeTo: "Contribute to",
    secureTransaction: "Simulated secure Sadaqah transaction flow",
    remaining: "Remaining",
    cash: "Cash",
    resource: "Resource",
    amountEgp: "Amount (EGP)",
    resourceType: "Resource Type",
    notesOptional: "Notes (Optional)",
    confirmDonation: "Confirm Donation",
    jazakAllahu: "Jazakallahu Khairan!",
    donationSuccess: "Your contribution has been recorded successfully. The beneficiary will be notified.",
    backToDashboard: "Back to Dashboard",
    viewProfile: "View Profile",

    // Beneficiary Profile (Donor View)
    backToDashboardLink: "Back to Dashboard",
    categoryLabel: "Category",
    fullyCovered: "Fully Covered",
    approxLocation: "Neighborhood (Approx. Location)",
    verifiedProfile: "Verified Needs Profile",
    monthlyTrackerTitle: "Monthly Required Support Tracker",
    receivedSoFar: "Received So Far",
    remainingGap: "Remaining Gap",
    monthlyTargetCap: "Monthly Target Cap",
    caseBackground: "Case Background & Social Worker Notes",
    contributeSupportFunds: "Contribute Support Funds",
    fullyFundedThisMonth: "This beneficiary has already received the full eligible monthly support for this month.",
    openChat: "Open Chat",
    caseNotFound: "Case profile not found",
    caseNotFoundDesc: "This profile is not active, pending approval, or has been hidden by admins.",

    // Footer
    footerHome: "Home",
    footerAbout: "About Governance",
    footerPrivacy: "Privacy Principles",
    footerTerms: "Terms of Service",
    footerCopyright: "KhairLink Sadaqah Governance. Ensuring aid reaches the truly eligible, with dignity.",
    footerVersion: "Auditable Log Layer v1.0.0",

    // Login
    loginTitle: "Welcome back",
    loginSubtitle: "Log in to the Sadaqah Governance Platform",
    emailLabel: "Email Address",
    passwordLabel: "Password",
    accessAccount: "Access Account",
    dontHaveAccount: "Don't have an account?",
    createAccountLink: "Create an account",
    demoCredentials: "Demo Credentials (Password: password123)",
    demoSuperAdmin: "Super Admin",
    demoDonor: "Donor (Sadaqah Giver)",
    demoCharity: "Charity (Resala Association)",
    demoBenApproved: "Beneficiary (Approved / Widow Case)",
    demoBenPending: "Beneficiary (Onboarding Pending)",
    loginFailed: "Failed to sign in. Please check your credentials.",
    errorTryAgain: "An error occurred. Please try again.",

    // Register
    registerTitle: "Create your account",
    registerSubtitle: "Join the KhairLink Sadaqah Governance Platform",
    roleDonor: "Donor",
    roleBeneficiary: "Beneficiary",
    roleCharity: "Charity",
    helpDonor: "You will be able to browse needy families nearby, make contributions, and track receipts.",
    helpBeneficiary: "Your profile remains 100% private. Donors see an anonymous code and display name only.",
    helpCharity: "Your account requires admin verification of license documents before you can log in fully.",
    fullNameLabel: "Full Name / Representative Name",
    choosePasswordLabel: "Choose Password",
    charityDetailsHeader: "Charity Organization Details",
    charityNameLabel: "Charity Name",
    licenseLabel: "License Number / Reg ID",
    phoneLabel: "Contact Phone Number",
    missionLabel: "Brief Mission Description",
    createAccountBtn: "Create Account",
    alreadyHaveAccount: "Already have an account?",
    loginInstead: "Log in instead",
    registrationFailed: "Registration failed.",
    errorRegistration: "An error occurred during registration.",

    // Map Extra
    searchPlaceholder: "Search by code, name, area...",
    loadingLocalCases: "Loading local cases...",
    noCasesFound: "No matching cases found within search radius. Try dragging the map or expanding the range.",
    egpRemaining: "EGP Remaining",
    critical: "Critical",
    high: "High",
    medium: "Medium",
    low: "Low",
    approxBoundary: "Approx. Boundary",
    coveredProgress: "Covered Progress:",
    members: "Members",
    kids: "Kids",
    score: "Score",

    // Tickets
    navTickets: "Tickets & Support",
    ticketsTitle: "Complaints & Suggestions",
    ticketsSubtitle: "Submit complaints or suggestions to improve our governance network. All responses are audited.",
    newTicket: "Submit New Ticket",
    myTickets: "My Tickets",
    allTickets: "All Tickets (Admin)",
    ticketType: "Type",
    ticketSubject: "Subject",
    ticketMessage: "Message",
    ticketStatus: "Status",
    ticketAdminReply: "Admin Reply",
    typeComplaint: "Complaint",
    typeSuggestion: "Suggestion",
    typeBug: "Bug Report",
    submitTicket: "Submit Ticket",
    ticketSuccess: "Ticket submitted successfully!",
    replyPlaceholder: "Write admin reply...",
    updateTicket: "Reply & Update Status",
    statusOpen: "Open",
    statusInReview: "In Review",
    statusResolved: "Resolved",
    statusClosed: "Closed",
    noTickets: "No tickets found.",

    // Charity Intermediary
    donateToCharity: "Donate to a Charity",
    donateToCharityDesc: "Support a certified charity organization. The charity distributes these funds transparently to families in need.",
    selectCharity: "Select Charity Organization",
    donationAmount: "Donation Amount",
    donationNotes: "Notes (Optional)",
    charityDonationSuccess: "Donation to charity recorded successfully! Jazakum Allahu Khairan.",
    btnDonateToCharity: "Donate to Charity",
    charityInfo: "Charity Details",
    licenseNumber: "License Number",

    // Bulk Donation
    bulkDonation: "Bulk Smart Donation",
    bulkDonationDesc: "Distribute a donation sum across multiple needy families instantly using filters.",
    filterCategory: "Category",
    filterArea: "Area",
    allAreas: "All Areas",
    allCategories: "All Categories",
    maxPerBeneficiary: "Max Limit Per Family (Optional)",
    bulkForceCapOverride: "Force override family cap limit",
    bulkForceCapOverrideDesc: "Force distribution even if monthly cap reached",
    distributeBtn: "Distribute Donation",
    bulkDonationSuccess: "Bulk donation processed successfully!",
    bulkSummaryTitle: "Distribution Results",
    familiesAssisted: "Families Assisted",
    totalDistributed: "Total Distributed",
    previewMatchingFamilies: "Matching families",
    previewWillReceive: "Est. support per family",
    noBeneficiariesMatched: "No matching active beneficiaries found.",
    aiAssessmentTitle: "AI Deep Learning Case Assessment",
    aiCategoryPredict: "AI Predicted Category",
    aiAmountPredict: "AI Predicted Monthly Amount",
    aiConfidence: "Prediction Confidence",
    aiLearningNote: "The Deep Learning model runs in real-time. With every manual adjustment, backpropagation fine-tunes the network's weights to adapt to your decisions.",
    sponsorsTitle: "Sponsors of Khair",
  },
  ar: {
    // Navbar
    navDashboard: "لوحة التحكم",
    navMapDiscovery: "خريطة الحالات",
    navBeneficiarySearch: "بحث الحالات",
    navMessages: "الرسائل",
    navHistory: "سجل العمليات",
    navResources: "المساعدات العينية",
    navAuditReports: "التقارير والتدقيق",
    navLogout: "تسجيل الخروج",
    navLogin: "تسجيل الدخول",
    navSignup: "إنشاء حساب",
    navGovernance: "الحوكمة",

    // Common
    welcome: "السلام عليكم",
    save: "حفظ",
    cancel: "إلغاء",
    confirm: "تأكيد",
    pending: "قيد الانتظار",
    delivered: "تم التسليم",
    failed: "فشل التسليم",
    egp: "جنيه",
    anonymous: "فاعل خير",
    loading: "جاري التحميل...",
    status: "الحالة",
    date: "التاريخ",
    details: "التفاصيل",
    confirmed: "مؤكد",
    cancelled: "ملغي",
    rejected: "مرفوض",
    en_route: "جاري التوصيل",
    transactions: "المعاملات",
    
    // Landing Page
    heroTitle: "عوكمة وإدارة الصدقات بشفافية تامّة",
    heroSubtitle: "منصة خير لينك تربط المتبرعين، المستفيدين، والجمعيات الخيرية في شبكة آمنة تحمي الخصوصية مع وضع حدود قصوى للمساعدات الشهرية.",
    getStarted: "ابدأ الآن",
    learnMore: "اقرأ المزيد",
    impactTitle: "سجل التأثير والشفافية المباشر",
    beneficiariesRegistered: "المستفيدين المسجلين",
    totalDonations: "إجمالي أموال الصدقات الموجهة",
    deliveryRate: "نسبة نجاح تسليم المساعدات",
    
    // Chat
    secureShield: "تم تفعيل درع الحماية الخصوصية",
    typeMessage: "اكتب رسالتك بشكل آمن...",
    supportChannels: "قنوات الدعم والتواصل",
    noChats: "لا توجد محادثات نشطة",
    safetyNotice: "إدارة الخصوصية: يتم حظر وحجب أرقام الهواتف أو الرقم القومي أو بيانات التواصل المباشر برمجياً لضمان كرامة المستفيدين.",
    
    // Map
    mapPortal: "بوابة استكشاف الحالات",
    searchRadius: "نطاق البحث الجغرافي",
    category: "الفئة المستحقة",
    familyDetails: "بيانات الأسرة",
    remainingNeed: "الاحتياج المتبقي",
    donateDirect: "تبرع مباشر",
    anonymousDetails: "تفاصيل مبهمة",
    privacyShielded: "حماية الخصوصية: إحداثيات المستفيدين معماة بنسبة عشوائية تتراوح بين 100 إلى 300 متر.",
    matches: "الحالات المطابقة",
    resetFilters: "إعادة ضبط التصفية",
    onlyShowNeeding: "أظهر فقط الحالات التي تحتاج لدعم عاجل هذا الشهر",
    
    // Transactions
    ledgerTitle: "سجل التدقيق المالي والعيني للصدقات",
    ledgerSubtitle: "سجل معاملات شفاف يربط عمليات توزيع المساعدات مع الحدود القصوى النشطة.",
    totalRegistry: "إجمالي القيود المسجلة",
    totalTransactedValue: "إجمالي قيمة المساعدات الموجهة",
    pendingGovernance: "بانتظار مراجعة الحوكمة",

    // Dashboards Common & Donor
    welcomeBack: "الحمد لله، مرحباً بعودتك",
    welcomeName: "مرحباً بك",
    donorPrivacyNotice: "يتم تتبع تبرعاتك بشفافية تامة مع إخفاء الإحداثيات الجغرافية الدقيقة حمايةً لخصوصية المستفيدين.",
    discoverCases: "استكشف الحالات القريبة منك",
    totalContributed: "إجمالي مساهماتك",
    directSupportFunds: "أموال دعم مباشر",
    confirmedPending: "التبرعات المؤكدة والمعلقة",
    directImpact: "الأثر المباشر",
    familiesSupported: "أسر تم دعمها",
    activeUrgentCases: "حالات الدعم العاجل النشطة",
    browseMap: "تصفح الخريطة",
    allTargetsCovered: "تم تغطية جميع أهداف الدعم الشهري بالكامل!",
    viewDetails: "عرض التفاصيل",
    support: "تقديم الدعم",
    contributionsHistory: "سجل مساهماتك",
    noDonationsYet: "لم تقم بأي تبرعات بعد.",
    startExploring: "ابدأ باستكشاف الحالات المحلية الموثقة بالقرب منك.",
    uniqueCasesSupported: "عائلات مميزة تم دعمها",

    // Beneficiary Dashboard
    caseId: "رقم الحالة",
    chatRoom: "غرفة المحادثة",
    pendingAuditTitle: "الحساب قيد تدقيق الحوكمة",
    pendingAuditDesc: "المستندات التي قمت بتقديمها قيد المراجعة حالياً من قبل مشرفي المنصة. بمجرد اكتمال التحقق وتغيير حالة ملفك الشخصي إلى 'مقبول'، ستصبح قابلاً للبحث من قبل المتبرعين والجمعيات المحلية.",
    rejectedTitle: "تم رفض الطلب",
    rejectedDesc: "للأسف، تم رفض طلب تقييم الاحتياجات الخاص بك لأنه لم يستوف معايير المنصة أو يحتوي على معلومات غير صحيحة. يرجى التواصل مع الدعم الفني للمزيد من التفاصيل.",
    monthlySupportTracker: "متتبع الدعم الشهري",
    currentMonth: "الشهر الحالي",
    progressToTarget: "التقدم نحو الهدف",
    percentMet: "مستوفى",
    monthlyCapTarget: "الحد الأقصى المستهدف شهرياً",
    receivedAid: "الدعم المستلم",
    remainingNeeded: "المتبقي المطلوب",
    socioEconomicScore: "مؤشر الوضع الاجتماعي والاقتصادي",
    assignedBracket: "الفئة المحددة",
    extremelyVulnerable: "وضع حرج جداً",
    veryNeedy: "بحاجة شديدة",
    needy: "بحاجة للدعم",
    limitedSupport: "حاجة محدودة",
    familyMembers: "أفراد الأسرة",
    childrenUnder18: "الأطفال دون 18 عاماً",
    employmentStatus: "حالة العمل",
    rentBurden: "عبء الإيجار",
    contributionsLedger: "دفتر المساهمات المالية",
    noContributionsYet: "لا توجد مساهمات مسجلة لحالتك بعد.",
    source: "المصدر",
    amount: "المبلغ",
    resourceDistributions: "سجل المساعدات العينية المستلمة",
    noResourcesYet: "لا توجد كراتين أو مساعدات عينية مرسلة إلى عنوانك بعد.",
    itemType: "نوع المادة",
    qty: "الكمية",
    notifications: "الإشعارات والتنبيهات",
    noNotifications: "لا توجد تنبيهات في صندوق الوارد.",
    locationUpdatedSuccess: "تم تحديث الموقع وإعدادات ظهور الخريطة بنجاح.",
    mapVisibilityTitle: "إعدادات الظهور والاكتشاف على الخريطة",
    showOnMapLabel: "إظهار حالتي على خريطة الاستكشاف العامة",
    showOnMapDesc: "إذا تم التفعيل، يمكن للمتبرعين العثور على حالتك بشكل تقريبي على الخريطة. يظل موقعك الدقيق مخفياً دائماً.",
    updateSettingsBtn: "تحديث الإعدادات",
    currentLocation: "منطقة السكن الحالية",
    executeCampaign: "إطلاق حملة توزيع",
    campaignSettings: "فلاتر استهداف الحملة",
    inventorySettings: "مخزون المستودع والمساعدات العينية",
    qtyPerFamily: "الكمية لكل أسرة",
    forceCapOverride: "فرض تجاوز الحد الأقصى للمستفيد (توزيع حتى لو تم استيفاء الحد الشهري)",
    deductStock: "خصم الكميات من المخزون الحالي",
    runCampaign: "تشغيل الحملة",
    campaignSuccess: "تم تنفيذ حملة التوزيع بنجاح!",
    stockAdjustment: "توريد / تعديل سريع للمخزون",
    addStockItem: "إضافة صنف مخزون جديد",

    // Reports Dashboard
    governanceEngine: "محرك حوكمة المنصة",
    auditLogsAnalytics: "سجل عمليات الحوكمة والتحليلات",
    charityLogsDesc: "مراجعة سجلات النشاط وتحليلات الدعم الخاصة بجمعيتك.",
    adminLogsDesc: "الإشراف على توجيه التبرعات، حدود الدعم، وسجلات التدقيق على مستوى المنصة.",
    beneficiaryReach: "الوصول للمستفيدين",
    approvedActiveCases: "الحالات النشطة المعتمدة",
    cashSupportLedger: "سجل الدعم المالي",
    directDonationsProcessed: "تبرع مالي مباشر تم معالجته",
    physicalAidValue: "قيمة المساعدات العينية",
    distributionsCompleted: "عمليات توزيع عيني تم إكمالها",
    totalAidValueRouted: "إجمالي قيمة المساعدات الموجهة",
    directSupportLogistics: "قيمة الدعم المباشر والخدمات اللوجستية",
    vulnerabilityRatio: "نسب الفئات المستحقة",
    scoringDefinition: "تعريف التقييم",
    scoringDefinitionDesc: "يتم حساب فئات الاستحقاق ديناميكياً بناءً على حالة الأسرة، مستوى الدخل، الديون، المشاكل الصحية، وظروف السكن.",
    auditTrail: "سجل تدقيق عمليات الحوكمة (آخر 50 إجراء)",
    timestamp: "التوقيت",
    actionEvent: "الحدث / الإجراء",
    operator: "المنفذ",
    operatorDetails: "التفاصيل",
    noAuditLogs: "لا توجد سجلات تدقيق تطابق المعايير.",

    // Resource Logistics
    resourceLogisticsPortal: "بوابة الخدمات اللوجستية والمساعدات العينية",
    resourceLogisticsDesc: "تتبع والتحقق من توزيع الموارد العينية (كراتين الطعام، الأدوية، المستلزمات) للأسر المستفيدة المسجلة.",
    noResourceDistributions: "لا توجد عمليات توزيع عيني مسجلة",
    noResourceDistributionsDesc: "لا توجد عمليات تسليم بضائع عينية معلقة أو مكتملة في النظام.",
    resourceItem: "المادة العينية",
    beneficiaryFamily: "الأسرة المستفيدة",
    logisticsValue: "القيمة اللوجستية",
    estimatedMarketValue: "القيمة السوقية التقديرية",
    assignedDate: "تاريخ التكليف",
    distributingEntity: "الجهة الموزعة",
    logisticsNotes: "ملاحظات لوجستية",
    updateLogisticsStatus: "تحديث حالة التسليم",
    statusNotes: "ملاحظات الحالة",
    addTrackingNotes: "إضافة ملاحظات تتبع...",
    deliveryStatus: "حالة التوصيل",
    enRoute: "جاري التوصيل",
    deliveryFailed: "فشل التوصيل",
    refReferenceId: "الرقم المرجعي / المعرف",
    refBeneficiary: "المستفيد",
    refSourceContributor: "الجهة المساهمة",
    refTypeParticulars: "النوع والتفاصيل",
    refValueEgp: "القيمة (جنيه)",

    // Admin Dashboard
    systemAdmin: "إدارة النظام",
    adminWelcome: "أهلاً بعودتك",
    adminTagline: "طبقة الحوكمة: فحص حدود الصدقات، مراجعة الحالات، وحماية المنصة.",
    adminCore: "النواة الإدارية",
    cases: "الحالات",
    pendingVerification: "بانتظار التحقق",
    totalRaised: "إجمالي المُجمَّع",
    acrossCashResources: "عبر الدعم النقدي والعيني",
    charities: "الجمعيات",
    pendingLicensing: "بانتظار مراجعة الترخيص",
    donors: "المتبرعون",
    activeContributors: "المساهمون النشطون",
    approvalQueue: "طابور الموافقة",
    auditCase: "مراجعة الحالة",
    verificationQueueClear: "طابور التحقق فارغ",
    verificationQueueClearDesc: "تمت مراجعة جميع الملفات المسجلة.",
    recentAuditLog: "سجل التدقيق الأخير",
    beneficiariesDirectory: "دليل المستفيدين",
    code: "الكود",
    displayName: "الاسم المعروض",
    legalName: "الاسم القانوني",
    targetCap: "الحد الأقصى",
    view: "عرض",
    navCharities: "الجمعيات",
    navDonors: "المتبرعون",

    // Admin Charities Directory
    charitiesDirectory: "دليل الجمعيات الخيرية",
    charitiesDirectoryDesc: "مراجعة تراخيص الجمعيات وتفاصيلها وإدارة حالة الوصول للمنصة.",
    backToAdminDashboard: "العودة للوحة الإدارة",
    searchCharityPlaceholder: "بحث باسم الجمعية أو رقم الترخيص...",
    noCharitiesMatch: "لا توجد جمعيات تطابق بحثك.",
    licenseId: "الترخيص",
    accountEmail: "بريد الحساب",
    registered: "تاريخ التسجيل",
    suspendAccess: "تعليق الوصول",
    approveOrganization: "اعتماد الجمعية",
    pendingVerificationBadge: "بانتظار التحقق",
    approvedBadge: "معتمدة",

    // Admin Donors Directory
    donorsDirectory: "دليل المتبرعين",
    donorsDirectoryDesc: "إدارة متبرعي المنصة، مراجعة إجماليات المساهمات، والتحكم في حالة الوصول.",
    searchDonorPlaceholder: "بحث المتبرعين بالاسم أو البريد الإلكتروني...",
    noDonorsMatch: "لا يوجد متبرعون يطابقون بحثك.",
    totalDonated: "إجمالي التبرعات",
    contributions: "المساهمات",
    joined: "تاريخ الانضمام",
    suspendedBadge: "موقوف",
    activeBadge: "نشط",
    reactivateAccount: "إعادة تفعيل الحساب",
    suspendDonor: "إيقاف المتبرع",

    // Admin Beneficiary Detail Page
    profileNotFound: "الملف الشخصي غير موجود",
    caseSummaryPublic: "ملخص الحالة والملف العام",
    displayNameLabel: "الاسم المعروض",
    generalNeighborhood: "الحي العام",
    caseStory: "قصة الحالة",
    confidentialProfile: "الملف القانوني السري",
    nationalId: "الرقم القومي",
    phone: "رقم الهاتف",
    email: "البريد الإلكتروني",
    exactAddress: "العنوان الكامل",
    contributionsLedgerAdmin: "دفتر المساهمات",
    contributor: "المساهم",
    type: "النوع",
    noContributionsCase: "لا توجد مساهمات مسجلة لهذه الحالة بعد.",
    physicalResourceDists: "توزيعات الموارد العينية",
    quantity: "الكمية",
    estimatedValue: "القيمة التقديرية",
    deliveryStatusLabel: "حالة التسليم",
    noDistributions: "لا توجد عمليات توزيع عيني مسجلة.",
    monthlySupportCycle: "دورة الدعم الشهري",
    supportProgress: "تقدم الدعم:",
    auditLogsComments: "سجلات التدقيق والتعليقات",
    noNotesLogged: "لم يتم تسجيل أي ملاحظات لهذه الحالة.",
    modifyCaseParameters: "تعديل بيانات الحالة",
    caseCode: "كود الحالة",
    overrideActive: "تجاوز الحد نشط",

    // Admin Review Page
    auditPending: "بانتظار التدقيق",
    reviewCase: "مراجعة",
    algorithmicAssessment: "التقييم الخوارزمي",
    recommendedCap: "الموصى به",
    caseSummarySection: "ملخص الحالة والملف العام",
    caseStoryLabel: "قصة الحالة / الملخص",
    confidentialLegalProfile: "الملف القانوني السري (للمشرف فقط)",
    contactPhone: "هاتف التواصل",
    homeAddress: "عنوان السكن",
    socioEconomicMetrics: "مقاييس التقييم الاجتماعي والاقتصادي",
    monthlyIncome: "الدخل الشهري",
    dependents: "عدد المعالين",
    childrenUnder18Short: "أطفال دون 18",
    employment: "العمل",
    housingTenure: "نوع السكن",
    monthlySupportReceived: "الدعم الشهري المستلم",
    outstandingDebts: "الديون المستحقة",
    chronicIllnesses: "الأمراض المزمنة / الحالة الصحية:",
    submittedDocuments: "المستندات المقدمة للتحقق",
    downloadScan: "تحميل المستند",
    governanceDecision: "لجنة قرار الحوكمة",
    decisionAction: "إجراء القرار",
    approveCase: "قبول الحالة",
    rejectApplication: "رفض الطلب",
    needCategoryBracket: "فئة الاحتياج",
    categoryA: "فئة أ - وضع حرج جداً",
    categoryB: "فئة ب - بحاجة شديدة",
    categoryC: "فئة ج - بحاجة للدعم",
    categoryD: "فئة د - حاجة محدودة",
    monthlyCapSupport: "الحد الأقصى للدعم الشهري (جنيه)",
    adminCapOverride: "تجاوز حد المشرف",
    adminCapOverrideDesc: "إذا تم التفعيل، يُسمح لهذه الحالة باستقبال تبرعات تتجاوز الحد الشهري دون إخفائها.",
    auditorComments: "تعليقات المدقق / ملاحظات المراجعة",
    documentReasonPlaceholder: "وثّق أسباب هذا القرار أو التوجيهات للأخصائيين الاجتماعيين...",
    commitAuditVerdict: "تثبيت حكم التدقيق",
    decisionAuditTrail: "سجل قرارات التدقيق",

    // Charity Dashboard
    charityPortal: "بوابة الجمعية الخيرية",
    licensePrefix: "الترخيص",
    approved: "✓ معتمدة",
    pendingApproval: "⏳ بانتظار الاعتماد",
    searchBeneficiaries: "بحث المستفيدين",
    totalContributedCharity: "إجمالي المساهمات",
    transactionsLabel: "المعاملات",
    familiesReached: "أسر تم الوصول إليها",
    activeCasesNeedingSupport: "الحالات النشطة التي تحتاج دعماً",
    allCasesFullySupported: "جميع الحالات مغطاة بالكامل!",
    contributionHistory: "سجل المساهمات",
    noContributions: "لا توجد مساهمات بعد.",

    // Donate Page
    backToCaseProfile: "العودة لملف الحالة",
    contributeTo: "التبرع لحالة",
    secureTransaction: "معاملة صدقة آمنة محاكاة",
    remaining: "المتبقي",
    cash: "نقدي",
    resource: "عيني",
    amountEgp: "المبلغ (جنيه)",
    resourceType: "نوع المساعدة",
    notesOptional: "ملاحظات (اختياري)",
    confirmDonation: "تأكيد التبرع",
    jazakAllahu: "جزاكم الله خيراً!",
    donationSuccess: "تم تسجيل مساهمتك بنجاح. سيتم إخطار المستفيد.",
    backToDashboard: "العودة للوحة التحكم",
    viewProfile: "عرض الملف الشخصي",

    // Beneficiary Profile (Donor View)
    backToDashboardLink: "العودة للوحة التحكم",
    categoryLabel: "الفئة",
    fullyCovered: "مغطى بالكامل",
    approxLocation: "الحي (موقع تقريبي)",
    verifiedProfile: "ملف احتياجات موثق",
    monthlyTrackerTitle: "متتبع الدعم الشهري المطلوب",
    receivedSoFar: "المستلم حتى الآن",
    remainingGap: "الفجوة المتبقية",
    monthlyTargetCap: "الحد الأقصى الشهري المستهدف",
    caseBackground: "خلفية الحالة وملاحظات الأخصائي الاجتماعي",
    contributeSupportFunds: "تقديم دعم مالي",
    fullyFundedThisMonth: "هذا المستفيد استلم بالفعل الدعم الشهري كاملاً لهذا الشهر.",
    openChat: "فتح المحادثة",
    caseNotFound: "ملف الحالة غير موجود",
    caseNotFoundDesc: "هذا الملف غير نشط أو بانتظار الموافقة أو تم إخفاؤه من قبل المشرفين.",

    // Footer
    footerHome: "الرئيسية",
    footerAbout: "حول الحوكمة",
    footerPrivacy: "مبادئ الخصوصية",
    footerTerms: "شروط الخدمة",
    footerCopyright: "خير لينك لحوكمة الصدقات. ضمان وصول المساعدات لمستحقيها بكرامة.",
    footerVersion: "طبقة سجل التدقيق إصدار v1.0.0",

    // Login
    loginTitle: "أهلاً بعودتك",
    loginSubtitle: "تسجيل الدخول إلى منصة حوكمة الصدقة",
    emailLabel: "البريد الإلكتروني",
    passwordLabel: "كلمة المرور",
    accessAccount: "دخول الحساب",
    dontHaveAccount: "ليس لديك حساب؟",
    createAccountLink: "إنشاء حساب جديد",
    demoCredentials: "بيانات التجربة المرجعية (كلمة المرور: password123)",
    demoSuperAdmin: "مدير عام النظام (سوبر أدمن)",
    demoDonor: "متبرع (مقدم صدقات)",
    demoCharity: "جمعية خيرية (جمعية رسالة)",
    demoBenApproved: "مستفيد (حالة أرملة معتمدة)",
    demoBenPending: "مستفيد (طلب التحقق قيد الانتظار)",
    loginFailed: "فشل تسجيل الدخول. يرجى التحقق من بياناتك.",
    errorTryAgain: "حدث خطأ ما. يرجى المحاولة مرة أخرى.",

    // Register
    registerTitle: "إنشاء حسابك",
    registerSubtitle: "انضم إلى منصة حوكمة صدقات خير لينك",
    roleDonor: "متبرع",
    roleBeneficiary: "مستفيد",
    roleCharity: "جمعية خيرية",
    helpDonor: "ستتمكن من تصفح الأسر المحتاجة القريبة منك، وتقديم المساهمات، وتتبع وصول التبرع.",
    helpBeneficiary: "ملفك الشخصي يظل سرياً بنسبة 100%. يرى المتبرعون الاسم المعروض ورمز الحالة فقط.",
    helpCharity: "يتطلب حسابك التحقق الإداري من وثائق الترخيص قبل أن تتمكن من العمل بالكامل.",
    fullNameLabel: "الاسم الكامل / اسم الممثل",
    choosePasswordLabel: "اختر كلمة المرور",
    charityDetailsHeader: "تفاصيل المؤسسة الخيرية",
    charityNameLabel: "اسم الجمعية الخيرية",
    licenseLabel: "رقم الترخيص / معرف التسجيل",
    phoneLabel: "رقم هاتف التواصل",
    missionLabel: "وصف موجز للمهمة والنشاط",
    createAccountBtn: "إنشاء الحساب",
    alreadyHaveAccount: "لديك حساب بالفعل؟",
    loginInstead: "سجل الدخول بدلاً من ذلك",
    registrationFailed: "فشل التسجيل.",
    errorRegistration: "حدث خطأ ما أثناء التسجيل.",

    // Map Extra
    searchPlaceholder: "البحث بالكود، الاسم، المنطقة...",
    loadingLocalCases: "جاري تحميل الحالات القريبة...",
    noCasesFound: "لم يتم العثور على حالات مطابقة في نطاق البحث المحدد. حاول سحب الخريطة أو زيادة النطاق.",
    egpRemaining: "جنيه متبقي",
    critical: "حرج",
    high: "مرتفع",
    medium: "متوسط",
    low: "منخفض",
    approxBoundary: "حدود تقريبية",
    coveredProgress: "التقدم المغطى:",
    members: "أفراد الأسرة",
    kids: "أطفال",
    score: "النتيجة",

    // Tickets
    navTickets: "الشكاوى والدعم",
    ticketsTitle: "الشكاوى والاقتراحات",
    ticketsSubtitle: "أرسل شكوى أو اقتراحاً للمساعدة في تحسين شبكة الحوكمة. جميع الردود والعمليات خاضعة للتدقيق.",
    newTicket: "إرسال تذكرة جديدة",
    myTickets: "تذاكري المعلقة",
    allTickets: "جميع تذاكر الدعم (مسؤول)",
    ticketType: "النوع",
    ticketSubject: "الموضوع",
    ticketMessage: "نص الرسالة",
    ticketStatus: "الحالة",
    ticketAdminReply: "رد الإدارة",
    typeComplaint: "شكوى",
    typeSuggestion: "اقتراح",
    typeBug: "تقرير مشكلة تقنية",
    submitTicket: "إرسال التذكرة",
    ticketSuccess: "تم تقديم التذكرة بنجاح!",
    replyPlaceholder: "اكتب الرد الإداري هنا...",
    updateTicket: "إرسال الرد وتحديث الحالة",
    statusOpen: "مفتوح",
    statusInReview: "قيد الدراسة",
    statusResolved: "تم الحل",
    statusClosed: "مغلق",
    noTickets: "لا توجد شكاوى أو اقتراحات معروضة.",

    // Charity Intermediary
    donateToCharity: "التبرع للجمعيات الخيرية",
    donateToCharityDesc: "ادعم جمعية خيرية معتمدة مباشرةً، لتتولى الجمعية توزيع هذه المساهمات للأسر الأكثر احتياجاً بشكل موثق.",
    selectCharity: "اختر الجمعية الخيرية",
    donationAmount: "مبلغ التبرع",
    donationNotes: "ملاحظات إضافية (اختياري)",
    charityDonationSuccess: "تم تسجيل التبرع للجمعية بنجاح! جزاكم الله خيراً.",
    btnDonateToCharity: "تبرع لجمعية خيرية",
    charityInfo: "تفاصيل الجمعية",
    licenseNumber: "رقم الترخيص",

    // Bulk Donation
    bulkDonation: "التبرع الذكي المتعدد",
    bulkDonationDesc: "وزع مبلغ تبرع كبير على مجموعة من الأسر المستحقة مباشرة وبنقرة واحدة باستخدام فلاتر ذكية.",
    filterCategory: "الفئة المستهدفة",
    filterArea: "المنطقة الجغرافية",
    allAreas: "كل المناطق",
    allCategories: "كل الفئات",
    maxPerBeneficiary: "أقصى مبلغ لكل أسرة (اختياري)",
    bulkForceCapOverride: "تجاوز الحد الأقصى للأسر المستهدفة",
    bulkForceCapOverrideDesc: "السماح بالتبرع حتى لو وصلت الأسرة للحد الأقصى الشهري",
    distributeBtn: "توزيع التبرعات الذكية",
    bulkDonationSuccess: "تم توزيع التبرع الجماعي بنجاح على الأسر المستحقة!",
    bulkSummaryTitle: "نتائج عملية التوزيع",
    familiesAssisted: "عدد الأسر المستفيدة",
    totalDistributed: "إجمالي التبرعات الموزعة",
    previewMatchingFamilies: "عدد الأسر المطابقة للفلاتر",
    previewWillReceive: "الدعم التقريبي لكل أسرة",
    noBeneficiariesMatched: "لا توجد حالات مطابقة نشطة وبحاجة لدعم هذا الشهر.",
    aiAssessmentTitle: "تقييم الذكاء الاصطناعي للحالة",
    aiCategoryPredict: "الفئة المتوقعة بالذكاء الاصطناعي",
    aiAmountPredict: "المبلغ الشهري المتوقع بالذكاء الاصطناعي",
    aiConfidence: "ثقة التوقع",
    aiLearningNote: "نموذج التعلم العميق يعمل في الوقت الفعلي. مع كل تعديل يدوي، تقوم التغذية الخلفية بضبط أوزان الشبكة لتتلاءم مع قراراتكم.",
    sponsorsTitle: "شركاء الخير ورعاة المنصة",
  }
};
