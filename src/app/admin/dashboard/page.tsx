        ...
        <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative overflow-hidden shadow-xl shadow-slate-200">
          ...
          <div className="space-y-1 relative z-10">
            ...
            <p className="text-slate-400 text-xs">
              Governance Layer: Sadaqah target checks, case audit approvals, and fraud protection.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Cases</span>
            </div>
            <div className="mt-4">
              <p className="text-2xl font-bold text-slate-900">{metrics.totalBens}</p>
              <p className="text-xs text-slate-400 mt-0.5">
                {metrics.pendingBens} pending verification
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Total Raised</span>
            </div>
            <div className="mt-4">
              <p className="text-2xl font-bold text-slate-900">
                {metrics.totalFunds.toLocaleString()} EGP
              </p>
              <p className="text-xs text-slate-400 mt-0.5">Across cash & resources</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Charities</span>
            </div>
            <div className="mt-4">
              <p className="text-2xl font-bold text-slate-900">{metrics.totalCharities}</p>
              <p className="text-xs text-slate-400 mt-0.5">
                {metrics.pendingCharities} pending licensing review
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Donors</span>
            </div>
            <div className="mt-4">
              <p className="text-2xl font-bold text-slate-900">{metrics.totalDonors}</p>
              <p className="text-xs text-slate-400 mt-0.5">Active contributors</p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              {metrics.pendingList.length === 0 ? (
                <div className="py-12 text-center space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400">All registered profiles have been audited.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {metrics.pendingList.map((p) => (
                    <div
                      key={p.id}
                      className="p-4 border border-slate-50 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors flex items-center justify-between gap-4"
                    >
          ...
          <div className="lg:col-span-7 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
            ...
            <div className="space-y-3.5">
              {metrics.recentAudits.map((log) => (
                <div
                  key={log.id}
                  className="flex items-start gap-3 p-3 border-b border-slate-50/50 text-xs"
                >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 text-xs font-bold">
                  ...
                  <th className="pb-3 text-left">Display Name</th>
                  <th className="pb-3 text-left">Legal Name</th>
                  <th className="pb-3 text-left">Category</th>
                  <th className="pb-3 text-left">Target Cap</th>
              ...
              <tbody>
                {metrics.allBens.map((b) => (
                  <tr key={b.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td className="py-3 font-bold text-slate-900">{b.monthlySupportCap} EGP</td>
                    <td className="py-3">
                      <span
