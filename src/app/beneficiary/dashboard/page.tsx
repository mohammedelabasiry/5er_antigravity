...
export default async function BeneficiaryDashboard() {
  ...
  const getStatusLabel = (status: BeneficiaryStatus) => {
    if (status === BeneficiaryStatus.FULLY_SUPPORTED_THIS_MONTH) return 'Fully Supported This Month';
    if (status === BeneficiaryStatus.PENDING_REVIEW) return 'Under Review';
    return status;
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            <p className="text-sm font-semibold text-emerald-600">Alhamdulillah, welcome back</p>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              {profile.displayName}
            </h1>
            <p className="text-xs text-slate-400 font-mono">Case ID: {profile.code}</p>
          </div>
        ...
        {profile.status === BeneficiaryStatus.PENDING_REVIEW && (
          <div className="p-5 bg-amber-50 border border-amber-200 text-amber-800 rounded-3xl flex items-start gap-3 shadow-sm">
            <ShieldAlert className="w-6 h-6 text-amber-600 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <h4 className="font-bold text-sm">Account Pending Governance Audit</h4>
              <p className="text-xs text-amber-700 leading-relaxed">
                Your submitted documents are currently being checked by platform administrators. Once verification completes and your profile status changes to **Approved**, you will become searchable by local donors and organizations.
              </p>
            </div>
          </div>
        )}
        ...
        {profile.status === BeneficiaryStatus.REJECTED && (
          <div className="p-5 bg-rose-50 border border-rose-200 text-rose-800 rounded-3xl flex items-start gap-3 shadow-sm">
            <ShieldAlert className="w-6 h-6 text-rose-600 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <h4 className="font-bold text-sm">Application Rejected</h4>
              <p className="text-xs text-rose-700 leading-relaxed">
                Unfortunately, your need evaluation registration was rejected as it did not satisfy platform need criteria or contains incorrect information. Please contact admin support for details.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="space-y-2 pt-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-emerald-700">{percentMet}% Met</span>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-slate-50 pt-6 mt-6">
              <div>
                <p className="text-[10px] sm:text-xs text-slate-400 font-medium">Monthly Cap Target</p>
                <p className="text-lg sm:text-xl font-bold text-slate-900 mt-0.5">{profile.monthlySupportCap} EGP</p>
              </div>
              <div>
                <p className="text-[10px] sm:text-xs text-slate-400 font-medium">Received Aid</p>
                <p className="text-lg sm:text-xl font-bold text-emerald-700 mt-0.5">+{receivedThisMonth} EGP</p>
              </div>
              <div>
                <p className="text-[10px] sm:text-xs text-slate-400 font-medium">Remaining Needed</p>
                <p className="text-lg sm:text-xl font-bold text-teal-600 mt-0.5">{remainingNeed} EGP</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
            ...
            <div className="flex items-center gap-4 py-3">
              <div className="w-16 h-16 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-100 flex items-center justify-center font-bold text-xl">
                {profile.evaluationScore}
              </div>
              <div>
                <p className="text-xs text-slate-400">Assigned Bracket</p>
                <p className="font-bold text-slate-800 text-lg">Category {profile.category}</p>
            ...
            <div className="border-t border-slate-50 pt-4 space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                <span className="text-slate-400">Children under 18:</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
            ...
            {contributions.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No contributions recorded yet for your case.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-50 text-slate-400 text-xs font-bold">
                  ...
                  <tbody>
                    {contributions.map((c) => (
                      <tr key={c.id} className="border-b border-slate-50/50 hover:bg-slate-50/30">
                        ...
                        <td className="py-3.5 font-bold text-slate-900">{c.amount} EGP</td>
                        <td className="py-3.5">
                          <span
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
            ...
            {resourceDeliveries.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No physical packages dispatched to your address.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-50 text-slate-400 text-xs font-bold">
                      <th className="pb-3 text-left">Item Type</th>
                  ...
                  <tbody>
                    {resourceDeliveries.map((rd) => (
                      <tr key={rd.id} className="border-b border-slate-50/50 hover:bg-slate-50/30">
                        ...
                        <td className="py-3.5 text-slate-600">{rd.quantity} unit(s)</td>
                        <td className="py-3.5">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
          ...
          {notifications.length === 0 ? (
            <p className="text-xs text-slate-400 py-4 text-center">No alerts in your inbox.</p>
          ) : (
            <div className="space-y-3">
              {notifications.map((n) => (
                <div
                  key={n.id}
                  className="p-3 border border-slate-50 rounded-xl bg-slate-50/50 flex items-start gap-2.5"
                >
