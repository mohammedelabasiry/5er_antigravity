  ...
  return (
    <div className={`flex-1 bg-slate-50/50 py-8 px-4 sm:px-6 lg:px-8 ${isRtl ? 'text-right' : 'text-left'}`}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          <ArrowLeft className={`w-4 h-4 ${isRtl ? 'rotate-180' : ''}`} /> {t('backToDashboardLink')}
        </Link>
        ...
        <div className={`bg-white border border-slate-100 p-6 rounded-3xl shadow-sm flex items-center gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900">{t('navBeneficiarySearch')}</h1>
            <p className="text-xs text-slate-500">
              {isRtl 
                ? 'تصفح الحالات المعتمدة ونسق عملية توزيع الدعم والمساعدات'
                : 'Browse verified cases and coordinate support distribution'}
            </p>
          </div>
        </div>
            ...
            return (
              <div key={b.id} className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4">
                <div className="space-y-2">
                  <div className={`flex justify-between items-center ${isRtl ? 'flex-row-reverse' : ''}`}>
                    ...
                    <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${b.status === 'FULLY_SUPPORTED_THIS_MONTH' ? 'bg-teal-50 text-teal-700' : 'bg-emerald-50 text-emerald-700'}`}>
                      {b.status === 'FULLY_SUPPORTED_THIS_MONTH' 
                        ? (isRtl ? 'مكتمل الدعم' : 'Supported') 
                        : `${t('category')} ${b.category}`}
                    </span>
                  </div>
                  <h4 className="font-bold text-slate-800 text-sm">{b.displayName}</h4>
                  <p className={`text-[10px] text-slate-505 flex items-center gap-1 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <MapPin className="w-3 h-3 text-emerald-600" />
                    {b.areaName}
                  </p>
                  <p className="text-[11px] text-slate-500 line-clamp-2 leading-relaxed">{b.caseSummary}</p>
                  <div className="space-y-1 pt-2 text-xs">
                    <div className={`flex justify-between ${isRtl ? 'flex-row-reverse' : ''}`}>
                      <span className="text-slate-400">{t('remaining')}:</span>
                      <span className="font-bold text-emerald-800">{remaining} {t('egp')}</span>
                    </div>
                ...
                <Link
                  ...
                  className={`py-2.5 text-center bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-sm transition-colors flex items-center justify-center gap-1 ${isRtl ? 'flex-row-reverse' : ''}`}
                >
                  {isRtl ? 'عرض وتقديم الدعم' : 'View & Contribute'}
