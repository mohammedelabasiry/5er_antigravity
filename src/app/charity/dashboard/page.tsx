      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            <p className="text-sm font-semibold text-emerald-600">Charity Organization Portal</p>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">{charity.charityName}</h1>
            <p className="text-xs text-slate-400">License: {charity.licenseNumber} · {charity.isApproved ? '✓ Approved' : '⏳ Pending Approval'}</p>
          </div>
          <Link href="/charity/search" className="px-5 py-3 bg-gradient-to-r from-emerald-600 to-teal-500 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md">
            <Users className="w-4 h-4" /> Search Beneficiaries
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-tr from-emerald-900 to-slate-900 text-white p-6 rounded-3xl shadow-md space-y-2 relative overflow-hidden">
            <div className="flex justify-between items-center text-emerald-400"><Coins className="w-5 h-5" /><span className="text-[10px] font-bold uppercase">Total Contributed</span></div>
          ...
          <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-2">
            <div className="flex justify-between items-center text-slate-400"><TrendingUp className="w-5 h-5 text-emerald-600" /><span className="text-[10px] font-bold uppercase">Transactions</span></div>
            <p className="text-3xl font-bold text-slate-900">{contributions.length}</p>
          </div>
          <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-2">
            <div className="flex justify-between items-center text-slate-400"><Heart className="w-5 h-5 text-emerald-600" /><span className="text-[10px] font-bold uppercase">Families Reached</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800 flex items-center gap-2"><Heart className="text-rose-500 w-5 h-5" /> Active Cases Needing Support</h3>
            {activeCases.length === 0 ? <p className="text-xs text-slate-400 py-8 text-center">All cases are fully supported!</p> : (
              <div className="space-y-3">
                {activeCases.map(b => {
                  const remaining = Math.max(0, b.monthlySupportCap - b.monthlyReceivedAmount);
                  return (
                    <div key={b.id} className="p-4 border border-slate-50 rounded-2xl bg-slate-50/50 flex justify-between items-center gap-3">
                      <div className="space-y-0.5 min-w-0">
                        ...
                        <p className="text-[10px] text-emerald-700 font-semibold">Remaining: {remaining} EGP</p>
                      </div>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800 flex items-center gap-2"><Coins className="text-emerald-600 w-5 h-5" /> Contribution History</h3>
            {contributions.length === 0 ? <p className="text-xs text-slate-400 py-8 text-center">No contributions yet.</p> : (
              <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
                {contributions.map(c => (
                  <div key={c.id} className="p-3.5 border border-slate-50 rounded-2xl bg-slate-50/50 flex justify-between items-center text-xs">
                    <div>
