      ...
      <div className="flex-1 p-8 text-center flex flex-col justify-center items-center">
        <div className="max-w-md mx-auto space-y-4">
          ...
          <p className="text-slate-500 text-xs">
            This profile is not active, pending approval, or has been hidden by admins.
          </p>
          <Link
        ...
        <div className="bg-white border border-slate-100 shadow-sm rounded-3xl p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-50 pb-6">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 flex items-center gap-1 font-semibold">
                ...
                {profile.areaName} Neighborhood (Approx. Location)
              </p>
            </div>

            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-800 text-xs font-semibold rounded-full border border-emerald-100">
                <ShieldCheck className="w-4 h-4 text-emerald-600" />
                Verified Needs Profile
              </span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
              ...
              Monthly Required Support Tracker
            </h3>

            <div className="space-y-2">
              <div className="flex justify-between text-xs sm:text-sm font-semibold">
                <span className="text-slate-450">Received: {received.toLocaleString()} EGP</span>
                <span className="text-emerald-700">{percentMet}% Met</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100 text-xs mt-3">
              <div>
                <p className="text-slate-400">Monthly Target Cap</p>
                <p className="font-bold text-slate-900 text-sm sm:text-base mt-0.5">
                  {profile.monthlySupportCap.toLocaleString()} EGP
                </p>
              </div>
              <div>
                <p className="text-slate-400">Received So Far</p>
                <p className="font-bold text-emerald-700 text-sm sm:text-base mt-0.5">
                  {received.toLocaleString()} EGP
                </p>
              </div>
              <div>
                <p className="text-slate-400">Remaining Gap</p>
          ...
          <div className="space-y-3 pt-2">
            <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
              ...
              Case Background & Social Worker Notes
            </h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-50">
            {profile.status === 'FULLY_SUPPORTED_THIS_MONTH' ? (
              <div className="flex-1 p-3 bg-teal-50 border border-teal-150 text-teal-800 text-xs font-semibold rounded-xl text-center">
                This beneficiary has already received the full eligible monthly support for this month.
              </div>
            ) : (
              <Link
                href={
